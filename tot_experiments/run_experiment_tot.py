...
def check_model_access():
    try:
        ...
        raise RuntimeError(f"Model access error: {e}")
    ...
    return [
        {
            "role": "system",
            "content": (
                "You are an expert in formal languages and automata. "
                "Internally reason if needed, but output only the DFA JSON."
            )
        },
...
def run_single_branch(entry, branch_name, template):
    ...
    base = safe_filename(f"{regex_id}_{branch_name}_{ts}")
    ...
    save_json(
        os.path.join(OUTPUT_META_DIR, base + "_meta.json"),
        make_json_safe(meta)
    )
...
def main():
    check_model_access()
    dataset = load_json(os.path.join(SCRIPT_DIR, DATASET_PATH))

    prompt_templates = {
        k: load_text(os.path.join(SCRIPT_DIR, v))
        for k, v in PROMPT_FILES.items()
    }

    for entry in dataset:
        print(f"Processing entry {entry['id']}")
        for branch, template in prompt_templates.items():
            run_single_branch(entry, branch, template)
